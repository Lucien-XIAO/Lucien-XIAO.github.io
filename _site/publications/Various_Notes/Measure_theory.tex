...
\begin{titlepage}
    ...
    \includegraphics[width=1\linewidth]{figures/measure-theory-cover.jpg}
    \vfill
    \textit{Author:}\\
...
I completed these notes while pursuing my Master 2 (M2) in \textit{Probabilités et Modèles aléatoires} at Sorbonne Université during the summer of 2026. The profound and elegant nature of measure theory forms the bedrock of modern probability, and navigating it requires a guide that is both rigorous and accessible. 

The primary motivation behind this work is to introduce students and researchers worldwide to an exceptionally well-crafted French textbook: \textit{Théorie de l'intégration}, written by Marc Briane and Gilles Pagès (published by De Boeck Supérieur, 2017). In the Francophone academic world, this text is highly regarded for its pedagogical clarity, deep theoretical insights, and its ability to seamlessly bridge abstract integration theory with applied probability. 

However, as the original text is in French, I wanted to overcome the language barrier and make its brilliant exposition available to a broader English-speaking audience. My work in this document primarily consists of synthesizing the original French content and translating its core ideas into English. Furthermore, I have incorporated my own insights, derivations, and examples—highlighting the connections and intuitions that I found most valuable as an M2 probability student.

It is my hope that these notes serve not only as a faithful homage to Briane and Pagès's masterful work but also as a valuable, self-contained resource for anyone looking to master measure theory. Whether you are encountering Lebesgue integration for the first time or seeking a rigorous brush-up for advanced probabilistic modeling, I hope you find this document illuminating.
\vspace{1em}
\clearpage
\tableofcontents
\newpage
...
\begin{figure}[h]
	\centering
    \includegraphics[width=0.4\linewidth]{figures/measure-theory-borel-sets.png}
...
\begin{proof}
Since $(\overline{\mathbb{R}},\delta)$ is compact, the sequence $(x_n)$ has a cluster value (by the Bolzano--Weierstrass theorem).
Let $\ell$ be a cluster value of $(x_n)$.
There exists a subsequence $(x_{\varphi(n)})$ converging to $\ell$ in $(\overline{\mathbb{R}},\delta)$.
Given that $x_{\varphi(n)}\le \sup_{k\ge \varphi(n)} x_k$, and the right side decreases to $\limsup_{n} x_n$, we obtain (by the compatibility of the topology and the order on $\overline{\mathbb{R}}$) that
\[
...
\left\{
\begin{aligned}
&f_d\!\Bigl(\bigcup_{i\in I}A_i\Bigr)=\bigcup_{i\in I} f_d(A_i),\\
&f_d\!\Bigl(\bigcap_{i\in I}A_i\Bigr)\subset\bigcap_{i\in I} f_d(A_i)\quad \text{(with equality if $f$ is injective)},\\
&f_r^{-1}\!\Bigl(\bigcup_{j\in J}B_j\Bigr)=\bigcup_{j\in J} f_r^{-1}(B_j),\\
...
\begin{itemize}
    ...
    \[
    \mathbbm{1}_A:(X,\mathscr{A}) \longrightarrow (\{0,1\},\mathscr{P}(\{0,1\})),
    \quad x\mapsto
    \begin{cases}
    1 &\text{if } x\in A, \\
    0 &\text{if } x\notin A.
    \end{cases}
    \]
    We note that the function $\mathbbm{1}_A$ is measurable if and only if $A\in \mathscr{A}$.
\end{itemize}
\end{remark}
...
\begin{proof}
    ...
    Conversely, let $B \in \mathscr{B}$. It is clear that $B \cap Y' \in \mathscr{B}$ since $Y' \in \mathscr{B}$. Consequently, $f^{-1}(B) = f^{-1}(B \cap Y') \in \mathscr{A}$ by hypothesis. The statement about the trace $\sigma$-algebra is obvious: when $Y' \in \mathscr{B}$, it is precisely constituted by the elements of $\mathscr{B}$ contained in $Y'$. 
\end{proof}
...
From which it clearly follows, $\mathscr{B}(\mathbb{R}^2) = \sigma(\{U \times V, \, U, V \text{ open sets of } \mathbb{R}\})$. According to proposition 4.1, $f$ will be measurable if $f^{-1}(U \times V) \in \mathscr{A}$, $U, V$ open sets of $\mathbb{R}$. Now, $f^{-1}(U \times V) = \underbrace{f_1^{-1}(U)}_{\in \mathscr{A}} \cap \underbrace{f_2^{-1}(V)}_{\in \mathscr{A}} \in \mathscr{A}$. 
\end{proof}
...
\begin{proposition}
The set of measurable functions from $(X, \mathscr{A})$ to $(\mathbb{K}, \mathscr{B}(\mathbb{K}))$ equipped with the usual operations $+,\cdot,\times$ on functions is a $\mathbb{K}$-algebra\footnote{\textit{i.e.}, a $\mathbb{K}$-vector space for the operations $+,\cdot$ and a ring for the operations $+,\times$, these four operations verifying furthermore various natural compatibility relations trivially verified in the case of vector spaces of functions with values in $\mathbb{K}$, for which we refer to an appropriate textbook.}.
\end{proposition}
...
\end{proof}
\begin{example}
If $f : \mathbb{R} \to \mathbb{R}$ is differentiable, then $f' = \lim_n n(f(. + 1/n) - f)$ is Borel measurable.
\end{example}

\textbf{Complements and refinements:}
\begin{itemize}
    \item[\textbf{1.}] Assertion $(c)$ can be refined; indeed, if we do not assume \textit{a priori} that the sequence $(f_n)_{n \geq 1}$ converges, we know however that
$$ \{f_n \xrightarrow{(\overline{\mathbb{R}})} .\} := \{x \in X : \lim_n f_n(x) \text{ exists in } \overline{\mathbb{R}}\} \in \mathscr{A} $$
since
\begin{align*}
\{f_n \xrightarrow{(\overline{\mathbb{R}})} .\} &= {}^c\{\overline{\lim}_n f_n \neq \underline{\lim}_n f_n\} \\
&= {}^c \left( \bigcup_{r \in \mathbb{Q}} \left( \{\underline{\lim}_n f_n < r\} \cap \{\overline{\lim}_n f_n \geq r\} \right) \right) \in \mathscr{A}.
\end{align*}

Furthermore, the mapping $\lim_n f_n : \{f_n \xrightarrow{(\overline{\mathbb{R}})} .\} \to \overline{\mathbb{R}}$ is always measurable with respect to the $\sigma$-algebras $\mathscr{A} \cap \{f_n \xrightarrow{(\overline{\mathbb{R}})} .\}$ (trace $\sigma$-algebra) and $\mathscr{B}(\overline{\mathbb{R}})$.

    \item[\textbf{2.}] Assertion $(c)$ remains true if we replace $\overline{\mathbb{R}}$ by any metric space $(E,d)$. Indeed, let $f_n : (X, \mathscr{A}) \to (E, \mathscr{B}(E))$ be a sequence of measurable functions converging pointwise to a function $f$. For every closed set $F$ of $E$, we have
\begin{align*}
f^{-1}(F) &= \{x \in X : d(f(x), F) = 0\} = \{x \in X : \lim_n d(f_n(x), F) = 0\} \\
&= \bigcap_{p \geq 1} \bigcup_{N \in \mathbb{N}} \bigcap_{n \geq N} \left\{ x \in X : d(f_n(x), F) \leq \frac{1}{p} \right\} \in \mathscr{A}
\end{align*}
because the mapping $x \mapsto d(f_n(x), F)$, composed of the measurable function $f_n$ and the 1-Lipschitz function $e \mapsto d(e, F)$, is measurable from $(X, \mathscr{A})$ to $(\mathbb{R}_+, \mathscr{B}(\mathbb{R}_+))$.

    \item[\textbf{3.}] If the metric space $(E,d)$ is furthermore complete and separable\footnote{Such a space is sometimes called a Polish space, in homage to the many Polish mathematicians who showed the importance of such spaces, notably in Probability.}, one can also establish the measurability of $\{f_n \xrightarrow{(E)} .\}$. Indeed, if $(e_k)_{k \in \mathbb{N}}$ denotes a dense sequence in $(E,d)$, one easily verifies that
\begin{align*}
\{f_n \xrightarrow{(E)} .\} &:= \{x \in X : (f_n(x))_{n \geq 1} \text{ is Cauchy} \} \\
&= \bigcap_{k \geq 1} \bigcup_{\ell \geq 1} \bigcap_{m, n \geq \ell} \bigcup_{p \in \mathbb{N}} \left( \left\{ x \in X / d(f_m(x), e_p) \leq \frac{1}{k} \right\} \cap \cdots \right. \\
&\qquad \left. \cdots \left\{ x \in X / d(f_n(x), e_p) \leq \frac{1}{k} \right\} \right) \in \mathscr{A}.
\end{align*}

The spaces $\mathbb{R}, \mathbb{R}_+, \mathbb{C}, \mathbb{R}^d, \mathbb{C}^d$ are complete separable metric spaces.
\end{itemize}

\textbf{Exercise:} Drawing inspiration from the method proposed in the first complement above, show that if the functions $f, g : (X, \mathscr{A}) \to (\overline{\mathbb{R}}, \mathscr{B}(\overline{\mathbb{R}}))$ are measurable then $\{f = g\} \in \mathscr{A}$.

\subsection{Simple functions on a measurable space}

\begin{definition}
\textit{A function $f : (X, \mathscr{A}) \to (\mathbb{K}, \mathscr{B}(\mathbb{K}))$ is simple if it is measurable and takes only a finite number of values ($\mathbb{K} = \mathbb{R}$ or $\mathbb{C}$).}
\end{definition}

It is immediate that any simple function $f : (X, \mathscr{A}) \to (\mathbb{K}, \mathscr{B}(\mathbb{K}))$ can be written in the form
\begin{equation}\label{eq:simple_function}
f = \sum_{i \in I} \alpha_i \mathbbm{1}_{A_i}, \quad I \text{ finite, } \alpha_i \in \mathbb{K}, \ (A_i)_{i \in I} \text{ an } \mathscr{A}\text{-measurable partition of } X 
\end{equation}
where we mean, by $\mathscr{A}$-measurable partition, any partition of $X$ consisting of elements (possibly empty) of $\mathscr{A}$.

Conversely, any function of the form (\ref{eq:simple_function}) is simple. Furthermore, among all decompositions of type (\ref{eq:simple_function}) of a simple function $f$, there exists one and only one in which the $\alpha_i$ are pairwise distinct. In this case it is clear that $\{\alpha_i, i \in I\} = f(X)$, $A_i = \{f = \alpha_i\}$, $i \in I$, and we obtain the \textit{canonical form} of $f$:
$$ f = \sum_{\alpha \in f(X)} \alpha \mathbbm{1}_{\{f = \alpha\}}. $$

Simple functions play in the construction of the Lebesgue integral the role assigned to step functions in Riemann theory.

\begin{example}
\begin{itemize}
    \item[\textbf{1.}] A function $f : (X, \mathscr{P}(X)) \to \mathbb{K}$ is simple if and only if $\text{card}(f(X))$ is finite.

    \item[\textbf{2.}] The indicator function $f := \mathbbm{1}_A$ of a measurable set $A \in \mathscr{A}$ is a simple function whose canonical form is obviously given by
$$ f = 1 \times \mathbbm{1}_A + 0 \times \mathbbm{1}_{^c A}. $$

Other expressions of type (\ref{eq:simple_function}) are generally possible: thus, as soon as there exists $\tilde{A} \subset A, \tilde{A} \in \mathscr{A}, \tilde{A} \neq A, \emptyset$, the function $f$ is also written
$$ f = 1 \times \mathbbm{1}_{\tilde{A}} + 1 \times \mathbbm{1}_{A \setminus \tilde{A}} + 0 \times \mathbbm{1}_{^c A}. $$

    \item[\textbf{3.}] If $X = [a, b], \ (a < b)$ and $\mathscr{A} = \mathscr{B}([a, b])$, any step function $f$ is simple. Indeed (\textit{cf.} definition 1.1) it takes a finite number of values and, for all $\alpha \in f([a, b])$, the set $\{f = \alpha\}$ is a finite union of intervals of $[a, b]$ and therefore a Borel set of $[a, b]$. The converse is false, which illustrates the fact that the notion of a simple function is a generalization of that of a step function.
\end{itemize}
\end{example}

\begin{proposition}
\textit{The set}
$$ \mathscr{E}_{\mathbb{K}}(X, \mathscr{A}) := \{f : (X, \mathscr{A}) \to (\mathbb{K}, \mathscr{B}(\mathbb{K})), \ f \text{ simple} \} $$
\textit{is a lattice $\mathbb{K}$-algebra\footnote{\textit{i.e.}, stable under finite max and min.}.}
\end{proposition}
\begin{proof}
The zero function is obviously simple. On the other hand, let $f := \sum_{i \in I} \alpha_i \mathbbm{1}_{A_i}$ and $g := \sum_{j \in J} \beta_j \mathbbm{1}_{B_j}$ be two decompositions of type (5.1). If $\lambda \in \mathbb{K}$,
\begin{align*}
\lambda f + g &= \sum_{i \in I, j \in J} (\lambda \alpha_i + \beta_j) \mathbbm{1}_{A_i \cap B_j}, \\
f g &= \sum_{i \in I, j \in J} \alpha_i \beta_j \mathbbm{1}_{A_i \cap B_j}, \\
\max(f, g) &= \sum_{i \in I, j \in J} \max(\alpha_i, \beta_j) \mathbbm{1}_{A_i \cap B_j},
\end{align*}
etc., which are in turn decompositions of type (5.1) relative to the $\mathscr{A}$-measurable partition $(A_i \cap B_j)_{i \in I, j \in J}$.
\end{proof}

\textbf{Exercise:} Show that any function $f$ of the form $f = \sum_{i \in I} \alpha_i \mathbbm{1}_{A_i}, \alpha_i \in \mathbb{K}$, $A_i \in \mathscr{A}, i \in I$, $I$ finite, is simple.

\begin{theorem}[Fundamental approximation lemma]
Let $f : (X, \mathscr{A}) \to \mathbb{R}, \overline{\mathbb{R}}$ or $\mathbb{C}$, be measurable. There exists a sequence $(f_n)_{n \geq 1}$ of simple functions, such that, for all $x \in X$, $\lim_n f_n(x) = f(x)$. Furthermore,

\begin{itemize}
    \item[(a)] if $f \geq 0$, we can choose the sequence $(f_n)_{n \geq 1}$ to be increasing and non-negative, in the sense that
$$ \forall n \geq 1, \quad 0 \leq f_n \leq f_{n+1}. $$

    \item[(b)] If $f$ is bounded, we can choose the sequence $(f_n)_{n \geq 1}$ such that $f_n$ converges uniformly to $f$ (i.e. $\lim_n \sup_{x \in X} |f_n(x) - f(x)| = 0$).
\end{itemize}
\end{theorem}

\begin{proof}
\begin{itemize}
\item[(a)] Suppose $f \geq 0$ and let us set for all $n \in \mathbb{N}$,
$$ E_{n,k} := \left\{ \frac{k}{2^n} \leq f < \frac{k+1}{2^n} \right\}, \ k \in \{0, \dots, n2^n - 1\}, \text{ and } E_{n,\infty} := \{f \geq n\}. $$

The sets $E_{n,k}$ and $E_{n,\infty}$ belong to the $\sigma$-algebra $\mathscr{A}$ as inverse images of intervals of $\mathbb{R}$ by the measurable function $f$. We then define for all $n \in \mathbb{N}$,
$$ f_n := \sum_{k=0}^{n2^n - 1} \frac{k}{2^n} \mathbbm{1}_{E_{n,k}} + n \mathbbm{1}_{E_{n,\infty}}. $$

The functions $f_n$ are simple by construction. We verify that, if $x \in E_{n,k}$,
$$ f_{n+1}(x) = 
\begin{cases} 
f_n(x) & \text{if } \frac{2k}{2^{n+1}} \leq f(x) < \frac{2k+1}{2^{n+1}}, \\
f_n(x) + \frac{1}{2^{n+1}} & \text{if } \frac{2k+1}{2^{n+1}} \leq f(x) < \frac{2(k+1)}{2^{n+1}},
\end{cases} $$
and if $x \in E_{n,\infty}$,
$$ f_{n+1}(x) = 
\begin{cases} 
n + 1 > f_n(x) & \text{if } f(x) \geq n + 1, \\
\frac{n2^{n+1} + \ell}{2^{n+1}} \geq n & \text{if } \frac{n2^{n+1} + \ell}{2^{n+1}} \leq f(x) < \frac{n2^{n+1} + \ell + 1}{2^{n+1}}, \ 0 \leq \ell \leq 2^{n+1} - 1.
\end{cases} $$

We have thus well established that $f_n(x) \leq f_{n+1}(x)$ for all $x \in X$.

It is obvious by construction that, if $x \in \{f < n\}, \ 0 \leq f(x) - f_n(x) \leq 2^{-n}$. Consequently, for all $x \in \{f < n_0\}$, we have,
$$ \forall n \geq n_0, \quad 0 \leq f(x) - f_n(x) \leq 2^{-n} \to 0. $$

Consequently $f_n \to f$ on $\{f < +\infty\} = \bigcup_{k \geq 1} \{f < k\}$.

Finally, if $x \in \{f = +\infty\} = \bigcap_{n \in \mathbb{N}} \{f \geq n\}, \ f_n(x) = n \to +\infty$.

If $f$ is bounded by $M$, we observe that $\{f \geq n\} = \emptyset$ for $n > M$, hence the uniform convergence in this case, because $0 \leq f(x) - f_n(x) \leq 2^{-n}$ for all $x \in X$ as soon as $n > M$.

\item[(b)] \textit{Real case:} If $f$ takes values in $\overline{\mathbb{R}}$, we decompose it into $f := f^+ - f^-$ where $f^+ := \max(f, 0)$ and $f^- := \max(-f, 0)$. The functions $f^+$ and $f^-$ satisfy $f^\pm \geq 0$, $f^+ + f^- = |f|$. Thus, $f^+$ and $f^-$ are bounded if and only if $f$ is. Finally, we note that, at every point $x \in X$, either $f^+(x)$ or $f^-(x)$ is zero.

We then consider the sequences $f_n^+$ and $f_n^-$ relative to $f^+$ and $f^-$ constructed in point (a) and we set $f_n := f_n^+ - f_n^-$. The function $f_n$ is clearly simple and $f_n \to f$ as $n \to +\infty$. No indeterminate form can occur during the passage to the limit; indeed, $0 \leq f_n^\pm \leq f^\pm$ so, with $x \in X$ fixed, one of the two sequences $(f_n^+(x))_{n \geq 1}$ or $(f_n^-(x))_{n \geq 1}$ is identically zero. The convergence is furthermore uniform if $f$ is bounded since the functions $f^\pm$ are.

\textit{Complex case:} If $f$ takes complex values, we write $f = \Re(f) + i\Im(f)$. 
\end{itemize}
\end{proof}

\begin{remark}
The fundamental approximation lemma rests effectively on the idea developed in the preliminary “From Riemann to Lebesgue” consisting of approximating a function $f$ by regularly cutting the “axis” of ordinates, instead of the “axis” of abscissas (as for step functions).
\end{remark}
...
\begin{enumerate}[label=(\alph*),leftmargin=1.5em]
    \item Let $(X, \mathscr{A})$ be a measurable space. We call a (positive) measure on $(X, \mathscr{A})$ any mapping $\mu : \mathscr{A} \to \overline{\mathbb{R}}_+$ satisfying:
    \begin{itemize}
        \item[(i)] $\mu(\emptyset) = 0$,
        \item[(ii)] If $(A_n)_{n \geq 1}$ is a sequence of elements of $\mathscr{A}$, pairwise disjoint (possibly empty):
        $$ \mu\left(\bigcup_{n \geq 1} A_n\right) = \sum_{n \geq 1} \mu(A_n) \qquad [\text{property of } \sigma\text{-additivity}]. $$
    \end{itemize}
    \item If $\mu(X) < +\infty$, the measure $\mu$ is said to be finite or bounded. If $\mu(X) = 1$, $\mu$ is a probability measure.
\end{enumerate}
\end{definition}

\begin{remark}
The above hypotheses entail the ``simple additivity'' of the measure $\mu$: for any family $A_1, \dots, A_n$ of subsets of $\mathscr{A}$, pairwise disjoint,
$$ \mu(A_1 \cup \dots \cup A_n) = \mu(A_1) + \dots + \mu(A_n) $$
(we simply set $A_k := \emptyset$ for $k > n$ and apply jointly (i) and (ii)).
\end{remark}

\begin{example}
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=1.5em]
    \item The zero measure on $(X, \mathscr{P}(X))$: $\forall A \in \mathscr{P}(X), \ \mu(A) := 0$.
    \item The gross measure: $\mu(\emptyset) := 0$ and $\forall A \in \mathscr{P}(X), \ A \neq \emptyset, \ \mu(A) := +\infty$.
    \item The Dirac measure at point $a \in X$:
    \begin{equation}
    \forall A \in \mathscr{P}(X), \quad \mu(A) := \begin{cases} 1 & \text{if } a \in A \\ 0 & \text{if } a \notin A. \end{cases} \tag{6.1}
    \end{equation}
    The Dirac measure at $a$ is denoted $\delta_a$.
    \item The \textit{counting measure} on $(X, \mathscr{P}(X))$:
    $$ \forall A \in \mathscr{P}(X), \quad \mu(A) := \begin{cases} \text{card}(A) & \text{if } A \text{ is finite,} \\ +\infty & \text{otherwise.} \end{cases} \quad (^1) $$
    
    It is immediate that $\text{card}(\emptyset) = 0$; on the other hand, if $A_1, \dots, A_n$ are pairwise disjoint subsets of $X$, $\text{card}(A_1 \cup \cdots \cup A_n) = \text{card}(A_1) + \cdots + \text{card}(A_n)$. The $\sigma$-additivity clearly follows from this relation. This indeed makes $m$ a measure.
    
    The counting measure is also characterized by the fact that, for all $x \in X$, $m(\{x\}) = 1$.
    
    This example is important because it makes the connection between integration and summable families indexed by $X$, and between integration and series when $X = \mathbb{N}$.
    
    The above examples are all defined on the trivial $\sigma$-algebra $\mathscr{P}(X)$ of all subsets of $X$. These measures exist \textit{a fortiori} by restriction to any $\sigma$-algebra $\mathscr{A}$ on $X$. On the other hand, it is generally not possible to define a measure on the entirety of $\mathscr{P}(X)$.

    \item Let $\mu$ be a measure on $(X, \mathscr{A})$ and $B \in \mathscr{A}$. We define the \textit{trace measure} of $\mu$ on $B$ by $\mu_B := \mu(\cdot \cap B)$. If $\mu(B) \in \mathbb{R}_+^*$, we define the \textit{conditional measure} given $B$ by
    $$ \mu(\cdot / B) := \frac{\mu(\cdot \cap B)}{\mu(B)}. $$

    \item The \textit{Lebesgue measure} on $\mathbb{R}^d$: this example is, by far, the most important of the list. It is indeed the Lebesgue measure that is the basis for the extension of the Riemann integral to Borel functions from $\mathbb{R}$ to $\mathbb{R}$, and then from $\mathbb{R}^d$ to $\mathbb{R}$.
    
    A few preliminaries are necessary to state the theorem of existence and characterization of the Lebesgue measure: let $a \in \mathbb{R}^d$ and $A \in \mathscr{B}(\mathbb{R}^d)$, we denote $a + A := \{a + x, x \in A\}$. The mapping $\tau_a : x \mapsto x - a$ being clearly continuous from $\mathbb{R}^d$ to $\mathbb{R}^d$, hence Borel measurable, the set $a + A = \tau_a^{-1}(A) \in \mathscr{B}(\mathbb{R}^d)$.
\end{enumerate}
\end{example}

\begin{theorem}
There exists a unique measure on $(\mathbb{R}^d, \mathscr{B}(\mathbb{R}^d))$ called the \textit{Lebesgue measure} on $\mathbb{R}^d$ and denoted $\lambda_d$ satisfying:
\begin{itemize}
    \item[(i)] $\lambda_d([0, 1]^d) = 1$,
    \item[(ii)] $\forall a \in \mathbb{R}^d, \ \forall A \in \mathscr{B}(\mathbb{R}^d), \quad \lambda_d(a + A) = \lambda_d(A)$.
\end{itemize}
\end{theorem}

We will often denote by the simple letter $\lambda$ the Lebesgue measure on the real line $\mathbb{R}$. We will temporarily assume the existence of $\lambda_d$. Initially, we will simply show that, in dimension 1, the Lebesgue measure $\lambda(I)$ of an interval $I$ coincides with its length (\textit{cf.} paragraph 6.1.2). This will nevertheless allow us to establish the uniqueness part of the theorem (\textit{cf.} Application 6.1). A first approach to the construction of $\lambda$ on $\mathbb{R}$ as a consequence of Carathéodory's theorem is proposed in section 6.3. The complete construction is addressed in section 6.5.2, after the detailed proof of Carathéodory's theorem. It can be skipped on a first reading.

The Lebesgue measure $\lambda_d$ on $\mathbb{R}^d$ will be constructed even further on, in chapter 11, when we tackle the notion of product measure. This phase will be the subject of section 11.2.2.
